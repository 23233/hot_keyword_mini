...
func RegisterAdminRoutes(party iris.Party) {
	...
	adminParty.Get("/production-readiness", func(ctx iris.Context) {
		report, err := services.CheckProductionReadiness(ctx.URLParam("app_id"), config.Cfg)
		if err != nil {
			ctx.StatusCode(iris.StatusBadRequest)
			_ = ctx.JSON(iris.Map{"code": 400, "msg": err.Error()})
			return
		}
		_ = ctx.JSON(iris.Map{"code": 0, "data": report})
	})
